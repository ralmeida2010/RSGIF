...
fogosClosed <- fogos %>%
  ...
  rename(TipoIncendio = Tipo.x) %>%  # Rename Tipo.x to TipoIncendio
  rename(TipoEstimado = Tipo.y) %>%  # Rename Tipo.y to TipoEstimado
  mutate(HaHoraEstimado = AreaTotalEstimado / as.numeric(difftime(DHFimEstimado, DHInicio, units = "hours"))) %>%  # Calculate HaHoraEstimado
  mutate(
    Distrito = replace(Distrito, Distrito == "Viana Do Castelo", "Viana do Castelo"),  # Replace "Viana Do Castelo"
    across(everything(), ~ ifelse(is.infinite(.), 0.0000001, .))  # Replace inf with 0.0000001 in all columns
  )
...
fogosClosed <- fogosClosed %>%
  select(-...1, -...2)
fogosClosed <- fogosClosed %>%
  mutate(DHInicio = as.POSIXct(DHInicio, format = "%Y-%m-%d %H:%M:%S"),
         DHFim = as.POSIXct(DHFim, format = "%Y-%m-%d %H:%M:%S"),
         DHFimEstimado = as.POSIXct(DHFimEstimado.y, format = "%Y-%m-%d %H:%M:%S"),
         DH1Intervencao = as.POSIXct(DH1Intervencao, format = "%Y-%m-%d %H:%M:%S"),
         DHResolucao = as.POSIXct(DHResolucao, format = "%Y-%m-%d %H:%M:%S"),
         DHConclusao = as.POSIXct(DHConclusao, format = "%Y-%m-%d %H:%M:%S"),
         )

fogosClosed <- fogosClosed %>%
  select(-DHFimEstimado.x, -DHFimEstimado.y)
